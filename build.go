...
func buildZip() {
	...
	zipFile(filename, []archiveFile{
		{"README.md", name + "/README.txt"},
		{"LICENSE", name + "/LICENSE.txt"},
		{"stocksim.exe", name + "/stocksim.exe"},
	})
	log.Println(filename)
}
...
func clean() {
	...
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/github.com/w0uld", goos, goarch)))
}
...
func archiveName() string {
	return fmt.Sprintf("stocksim-%s-%s", buildArch(), version())
}
